    ...
    private func stopCaptureSession() {
       ...
       private func drawDetectionResults(_ results: [VNRecognizedObjectObservation]) {
        ...
        private func showDetailPopup(for signClass: String, confidence: Float) {
            ...
            let icon = getIconForDetection(signClass: signClass)
            imageView.image = icon

            let formattedClassName = signClass.lowercased().replacingOccurrences(of: " ", with: "_")

            if icon == UIImage(systemName: "exclamationmark.triangle") {
                ...
                case _ where formattedClassName.contains("check"),
                    ...
                    print("Set tint to ORANGE")
                    
                default:
                    imageView.tintColor = .systemYellow
                    print("Set tint to YELLOW")
                }
            }
            ...
            actionLabel.attributedText = getAttributedActionFor(signClass: signClass)
        ...
        private func getAttributedActionFor(signClass: String) -> NSAttributedString {
            let actionText = getActionFor(signClass: signClass)
            let attributedString = NSMutableAttributedString(string: actionText)
            

            let boldFont = UIFont.systemFont(ofSize: 16, weight: .bold)
            let regularFont = UIFont.systemFont(ofSize: 16)
            
            if actionText.hasPrefix("Action:") {
                attributedString.addAttribute(.font, value: boldFont, range: NSRange(location: 0, length: 7))
                attributedString.addAttribute(.font, value: regularFont, range: NSRange(location: 7, length: actionText.count - 7))
            } else {
                attributedString.addAttribute(.font, value: regularFont, range: NSRange(location: 0, length: actionText.count))
            }
